...
pub fn init(allocator: std.mem.Allocator, pixel_buffer: Buffer2D(PlatformOutPixelType)) !void {
    ...
    app.debug = true;
    app.zoom = 1;
    app.level_background = Assets.LevelBackgroundDescriptor.from(Vector2i.from(0, 0), Vector2i.from(239+1, 135+1));
...
pub fn update(platform: *Platform) !bool {
    ...
    // render the map
    try app.renderer_quads.add_map(Assets.map, app.level_background.tl, app.level_background.br, Vector2f.from(0,0));
    ...
    const level_list_top_left = Vector2f.from(10, h - h/5);
    ...
    const sprite_selection_bb = BoundingBox(f32).from(sprites_top, sprites_top-16*8, 10, 10+16*8);
    ...
    for (0..16) |j| {
        for (0..16) |i| {
            ...
            const pos = Vector2f.from(10 + ii*8, sprites_top - (jj+1)*8);
    ...
    if (sprite_selection_bb.contains(mouse_window)) {
        ...
        if (platform.l_click) app.selected_sprite = @intFromFloat(sprite_selected.x + (15 - sprite_selected.y)*16);
        ...
        const map_tiles_bb = blk: {
            var bb = app.level_background.to_bb_corrected();
            break :blk bb.offset(Vector2f.from(-bb.left, -bb.bottom));
        };
    ...
    {
        const sprite_selected = Vector2f.from(@floatFromInt(app.selected_sprite%16), @floatFromInt(16 - @divFloor(app.selected_sprite,16))).scale(8).add(Vector2f.from(10, 10-8));
...
pub const Assets = struct {
    ...
    pub const level_all_levels = LevelDescriptor.from(
        &Assets.LevelBackgroundDescriptor.from(Vector2i.from(0, 0), Vector2i.from(239+1, 135+1)),
