...
pub fn init(allocator: std.mem.Allocator) anyerror!void {
    ...
    state.animations_in_place = try AnimationSystem.init_capacity(allocator, 1000);
    state.particles = try Particles.init_capacity(allocator, 10000);
    state.rng_engine = std.rand.DefaultPrng.init(@bitCast(platform.timestamp()));
    state.random = state.rng_engine.random();
    state.doors = try std.ArrayList(Door).initCapacity(allocator, 64); // doors are set on load_level
    ...
    try load_level(Vec2(u8).from(21, 29), 0);
}
...
pub fn update(ud: *platform.UpdateData) anyerror!bool {
    ...
    if (ud.key_pressed('R')) try load_level(Vec2(u8).from(21, 29), ud.frame);
    ...
    const charged_jump = struct {
        var start_frame: ?usize = null;
        var jump_level: usize = 0;
    };

    if (ud.key_pressed('B')) {
        charged_jump.start_frame = ud.frame;
    }
    else if (!ud.keys['B'] and ud.keys_old['B']) { // key released
        charged_jump.jump_level = std.math.clamp(ud.frame - charged_jump.start_frame.?, 0, 60*3);
        const resulting_jump_factor: f32 = @floatFromInt(charged_jump.jump_level);
        state.player.physical_component.velocity.y = 0.1 * resulting_jump_factor/50;
        state.player.physical_component.velocity.x += 0.10  * resulting_jump_factor/50;
        state.player.jumps -= 1;
        // TODO sfx
        for (0..charged_jump.jump_level) |_| try particle_create(&state.particles, particles_generators.other(
            state.player.pos.add(Vector2f.from(0, 1.5)),
            2 + state.random.float(f32) * 2,
            Vector2f.from(((state.random.float(f32) * 2) - 1)*0.2, -0.05),
            100,
        ));
        charged_jump.start_frame = null;
    }

    if (charged_jump.start_frame) |value| {
        charged_jump.jump_level = std.math.clamp(ud.frame - value, 0, 60*3);
    }
    ...
    // HUD
    {
        ...
        try renderer.add_quad_from_bb(BoundingBox(f32).from(10, 10-5, 5, 5 + @as(f32, @floatFromInt(charged_jump.jump_level))), color);
        try renderer.flush_all();
    }
    ...
    if (state.debug) {
        ...
        try renderer.add_text(Vector2f.from(5, h - (text_height*11)), "particles {}", .{state.particles.stats().entities}, color);
        try renderer.add_text(Vector2f.from(5, h - (text_height*12)), "jump_level {}", .{charged_jump.jump_level}, color);
...
const EntitySystem = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !EntitySystem {
        return EntitySystem {
            .entities = try EntityStorage.init_capacity(allocator, 16)
        };
    }
